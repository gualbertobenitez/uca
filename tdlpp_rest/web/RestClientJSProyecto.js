...
// Model for Proyecto entity
    models.Proyecto = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/tdlpp_rest/api/proyecto/",
        idAttribute: 'proyectoId',
        defaults: {
            estado: "",
            nombre: "",
            comentarios: "",
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('proyectoId');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method == 'create') {
                options.url = 'http://localhost:8080/tdlpp_rest/api/proyecto/';
            }
    ...
    // Collection class for Proyecto entities
    models.ProyectoCollection = Backbone.Collection.extend({
        model: models.Proyecto,
        url: "http://localhost:8080/tdlpp_rest/api/proyecto/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.ProyectoCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-proyecto-list-itemis template identifier for item
                        templateName: '#tpl-proyecto-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.proyecto = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.proyecto,
                    // tpl-proyecto-details is template identifier for chosen model element
                    templateName: '#tpl-proyecto-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Proyecto();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-proyecto-details is a template identifier for chosen model element
                templateName: '#tpl-proyecto-details',
